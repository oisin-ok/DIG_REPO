  ...
  navbarPage("Digoxin Trial Data",
             ...
             tabPanel("Participant Information",
                      sidebarLayout(
                        ...
                        sidebarPanel(
                          selectInput("dem_var", "Choose a variable to view its distribution:", choices = every_var)
                        ), 
                        ...
                        tabPanel("Mixed Compare (Boxplot)",
                                 ...
                                 sidebarLayout(
                                   ...
                                   sidebarPanel(
                                     ...
                                     selectInput("base_var2", "Choose a condition to compare across:", choices = cat_vars), 
...
server <- function(input, output) {
  ...
  ####################### SERVER CODE TAB 1: Participant Information ##################################################################################################
  
  # Histogram
  output$histogram_plot <- renderPlotly({
    req(input$dem_var)
    if (input$dem_var %in% cont_vars) {
      plot_ly(dig.df, x = ~ get(input$dem_var), type = "histogram") %>%
        layout(title = paste("Distribution of", input$dem_var),
               xaxis = list(title = input$dem_var),
  ...
  output$pie_chart_plot <- renderPlotly({
    req(input$dem_var)
    if (input$dem_var %in% cat_vars) {
      plot_ly(dig.df, labels = ~ get(input$dem_var), type = "pie") %>%
        layout(title = paste("Distribution of", input$dem_var))
    }
  })
  ...
  output$boxplot_plot <- renderPlotly({
    req(input$dem_var)
    if (input$dem_var %in% cont_vars) {
      plot_ly(dig.df, y = ~ get(input$dem_var), type = "box") %>%
        layout(title = paste("Boxplot of", input$dem_var),
               yaxis = list(title = input$dem_var))
    }
  })
  ...
  #km_plot - Kaplan-Meier plot
  output$km_plot <- renderPlot({
    req(input$mortality_type == "Kaplan_Meir")
    
    km_fit <- survfit(Surv(Month, DEATH) ~ 1, data = dig.df)
    
    ggsurvplot(km_fit, data = dig.df, risk.table = TRUE)
  })
  ...
  output$table_dt <- DT::renderDataTable(DT::datatable({
    ...
    if (!is.null(input$hosp_buttons) && length(input$hosp_buttons) > 0) {
      data <- data[rowSums(data[, input$hosp_buttons, drop = FALSE]) == length(input$hosp_buttons), ]
    } else {
      data
    }
  },  rownames = FALSE,
